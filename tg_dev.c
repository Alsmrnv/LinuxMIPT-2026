...
static int fifo_send_request(const char *request, size_t len) {
    ...
    if (written < 0) {
        printk("failed to send request to %s fifo\n", write_fifo_path);
        return written;
    }

    printk("request: '%s' was sent to %s fifo\n", request, write_fifo_path);
...
static ssize_t fifo_read_response(char* out_buf, size_t size) {
    ...
    f = filp_open(read_fifo_path, O_RDONLY, 0);
    ...
    if (bytes_read < 0) {
        printk("failed to read from %s fifo\n", read_fifo_path);
        return bytes_read;
    }
    out_buf[bytes_read] = '\0';

    printk("response was read: %ld bytes\n", bytes_read);
...
static ssize_t tg_read(struct file* f, char __user* buf, size_t len, loff_t* off) {
    ...
    if (!response) {
        printk("can't allocate buffer for messenger response\n");
        return -1;
    }
    ...
    if (copy_to_user(buf, response, resp_len) > 0) {
        printk("can't copy response to user space\n");
        kfree(response);
        return -1;
    }
...
static ssize_t tg_write(struct file* f, const char __user* buf, size_t len, loff_t* off) {
    ...
    if (!msg) {
        printk("can't allocate buffer for message\n");
        return -1;
    }

    if (copy_from_user(msg, buf, len)) {
        printk("can't copy buffer from user space\n");
        kfree(msg);
        return -1;
    }
    msg[len] = '\0';
    ...
    if (!request) {
        printk("can't allocate buffer for request\n");
        return -1;
    }
    ...
    char* response = kmalloc(kBufSize, GFP_KERNEL);
    if (!response) {
        printk("can't allocate buffer messenger response\n");
        return -1;
    }
